...
export class Snapgrab {
    ...
    constructor(element: HTMLElement, options: SnapgrabOptions = {}) {
        ...
        this.wrapper = this.element.querySelector('[data-ref="wrapper"]') as HTMLElement
    ...
    bindEvents() {
        ...
        this.wrapper.addEventListener('touchend', (e) => this.handleMouseUp(e as unknown as MouseEvent))
    ...
    detectCurrentSlide() {
        const slideWidth = (this.wrapper.children[0] as HTMLElement).offsetWidth
    ...
    triggerSlideChangeEvent(slideIndex: number) {
    ...
    handleHeight() {
        const currentSlideElement = this.wrapper.children[this.state.currentSlide] as HTMLElement
        ...
        if (slideStyle.display.includes('flex')) {
            ...
            const columnHeights = Array.from(currentSlideElement.children).reduce((heights, child, index) => {
                heights[index % slideStyle.gridTemplateColumns.split(' ').length] += (child as HTMLElement).getBoundingClientRect().height
                return heights
    ...
    scrollSlides(direction: number) {
        const slideWidth = (this.wrapper.children[0] as HTMLElement)?.offsetWidth || 0
    ...
    updateActiveDot(startIndex: number, endIndex: number) {
        if (!this.dots) return
